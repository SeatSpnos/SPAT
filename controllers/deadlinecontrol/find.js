...
function tec( params, next ) {
	...
	model.tec.findAvailable(params.params.body.date, function( err, data ) {
	...
	function loop( callback ) {
		...
		model.OT.getEstado( temp2.OT, function( err, ot ) {
			...
			if( ot.length ) {
				...
				model.boni( temp2.OT, function( err, boni ) {

					temp2.BONIFICACAO = boni[0].BONIFICACAO
					temp2.OBS_CTL 		= boni[0].OBS_CTL
					temp2.SIT_CTL 		= boni[0].SIT_CTL
...
function render( params, next ) {
	...
	menus( function( err, rows ) {
		...
		params.res.render('./pages/deadlineControl/deadlinecontrol', {
