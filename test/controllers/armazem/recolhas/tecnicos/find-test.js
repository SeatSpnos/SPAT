...
describe('#Testing armazem recolhas-tecnicos', function() {
  describe('#Testing recolhas-tecnicos-find.js', function() {
    describe('With errors and without db', function() {
      ...
      it('it should return an error 500 when there is no connection to db', function (done) {
        request
          .get(`/armazem/recolhas/tecnicos/items/${createStock[0]}`)
    ...
    describe('With errors and db', function() {
	    ...
	    after(function (done) {
		  ...
		  it('it should return an error if request not found', function (done) {
		    request
		      .get(`/armazem/recolhas/tecnicos/items/${createStock[1].currentOwner}`)
		      .end(function(err, res) {
		        assert.isNotOk(err);
    ...
    describe('Without errors with db',function() {
    	...
    	it('it return the searched items by the currentOwner name', function (done) {
        request
          .get(`/armazem/recolhas/tecnicos/items/${createStock[0].currentOwner}`)
          .end(function (err, res) {
            assert.isNotOk(err);
